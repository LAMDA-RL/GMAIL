...
class Logger(LoggerBase):
    def __init__(self, parser=None, log_to_file=True, parameter=None, force_backup=False):
        if parameter:
            self.parameter = parameter
        else:
            self.parameter = Parameter(parser)
        # self.output_dir = os.path.join(get_base_path(), 'log_file', self.parameter.short_name)
        self.output_dir = os.path.join('log_file', self.parameter.short_name)
        if log_to_file:
        ...
        self.log(f"Logging data to {self.output_dir}", color='green')
    ...
    def get_model_output_path(parameter):
        output_dir = os.path.join('log_file', parameter.short_name)
