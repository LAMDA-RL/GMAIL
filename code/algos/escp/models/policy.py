...
class Policy(torch.nn.Module):
    def __init__(self, obs_dim, act_dim, up_hidden_size, up_activations, up_layer_type,
                 ...
                 rnn_fix_length, use_rmdm, share_ep, use_absorbing_state=False,
    ...
    def make_config_from_param(parameter):
        return dict(
            ...
            use_absorbing_state=parameter.use_absorbing_state
        )
    ...
    def inference_one_step(self, state, deterministic=True):
        ...
        with torch.no_grad():
            ...
            if self.use_absorbing_state:  # when test: input does not have the extra indicatar
                state = torch.hstack([state, torch.tensor([[0]]).to(device=self.device)])
