...
class EnvWorker:
    def __init__(self, parameter: Parameter, env_name='Hopper-v2', seed=0, policy_type=Policy,
                 ...
                 use_true_parameter=False, non_stationary=False, fix_env=None):
        ...
        if parameter.use_absorbing_state:
            obs_dim = obs_dim + 1
        self.policy = policy_type(obs_dim=obs_dim,
    ...
    def step(self, action, env_ind=None, render=False, need_info=False):
        ...
        info["env_params"] = list(self.env.cur_params.values())[0][-1]
        if render:
            self.env.render()
        if self.non_stationary:
            self.env_param_vector = self.env.env_parameter_vector
...
class EnvRemoteArray:
    def __init__(self, parameter, env_name, worker_num=2, seed=None,
                 ...
                 env_tasks=None, use_true_parameter=False, non_stationary=False, fix_env=None):
        self.env = gym.make(env_name)
        ...
        if use_remote:
            self.workers = [RemoteEnvWorker.remote(parameter, env_name, random.randint(0, 10000),
                                                   ...
                                                   use_true_parameter, non_stationary, fix_env) for _ in range(worker_num)]
        else:
