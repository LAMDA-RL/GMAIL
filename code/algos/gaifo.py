...
class GAIfO(GAIL):
    ...
    def _train_body(self, agent_states, agent_next_states, expert_states, expert_next_states):
        epsilon = 1e-8
        fake_logits = self.disc.forward(torch.cat((agent_states, agent_next_states), axis=1))
        if self.H == 1:
            real_logits = self.disc.forward(torch.cat((expert_states, expert_next_states), axis=1))
            loss = -(torch.mean(torch.log(real_logits + epsilon)) +
                   torch.mean(torch.log(1. - fake_logits + epsilon)))
        else:
            for h in range(self.H):
                expert_next_states_h = expert_next_states[:, h, :].squeeze()
                if h == 0:
                    real_logits = self.disc.forward(torch.cat((expert_states, expert_next_states_h), axis=1))
                    real_logits = ((self.beta) ** (self.H - h)) * torch.log(real_logits + epsilon)
                else:
                    real_logit = self.disc.forward(torch.cat((expert_states, expert_next_states_h), axis=1))
                    real_logit = ((self.beta) ** (self.H - h)) * torch.log(real_logit + epsilon)
                    real_logits = real_logits + real_logit
            loss = -(torch.mean(real_logits + epsilon) +
                   torch.mean(torch.log(1. - fake_logits + epsilon)))
        
        if self.use_gp:
            if len(expert_next_states.shape) == 3:
                expert_next_states = expert_next_states[:, 0, :]
            real_data = torch.cat((expert_states, expert_next_states), axis=1)
            fake_data = torch.cat((agent_states, agent_next_states), axis=1)
