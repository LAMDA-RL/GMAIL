...
class MemoryArray(object):
    def __init__(self, rnn_slice_length=32, max_trajectory_num=1000, max_traj_step=1050, 
                 fix_length=0, use_absorbing_state=False):
        self.memory = []
    ...
    def complete_traj(self, memory):
        ...
        if self.use_absorbing_state and not memory[-1].mask[0]:
            action = np.zeros(memory[-1].action.shape)
            absorbing_state = np.zeros(memory[-1].state.shape[0], )  # absorbing state
            absorbing_state[-1] = 1.
            # done=False is set to the absorbing state because it corresponds to
            # a state where gym environments stopped an episode.
            trainsition = Transition(absorbing_state, action, [-1.], absorbing_state, [0.], None, 
                                     memory[-1].task, memory[-1].env_param, memory[-1].action, [False], [0.])
            memory.append(trainsition)
