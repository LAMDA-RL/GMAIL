...
class NonstationaryEnv(Wrapper):
    ...
    def sample_tasks(self, n_tasks, dig_range=None, linspace=False, expert_tasks=None, train_mode=False):
        """
        ...
        param_sets = [] if expert_tasks is None or not train_mode else expert_tasks
        if expert_tasks is not None:
            expert_tasks_list = [tuple(list(env_param.values())[0]) for env_param in expert_tasks]
        ...
        while len(param_sets) < n_tasks:
            ...
            if 'body_mass' in self.rand_params:
                ...
                new_task_mass = tuple(np.round(new_params['body_mass'], 8))
                if expert_tasks is not None and new_task_mass in expert_tasks_list:
                    continue
            ...
            if 'body_inertia' in self.rand_params:
                ...
                new_task_iner = tuple(np.round(new_params['body_inertia'], 8))
                if expert_tasks is not None:
                    expert_tasks_clean = [task[-1][-1] for task in expert_tasks_list]
                if expert_tasks is not None and new_task_iner[-1][-1] in expert_tasks_clean:
                    continue
            ...
            if 'dof_damping' in self.rand_params:
                ...
                new_task_dof = tuple(np.round(new_params['dof_damping'], 8))
                if expert_tasks is not None and new_task_dof in expert_tasks_list:
                    continue
            ...
            if 'geom_friction' in self.rand_params:
                ...
                new_task_fric = np.round(new_params['geom_friction'], 8)
                if expert_tasks is not None:
                    expert_tasks_clean = [task[-1][-1] for task in expert_tasks_list]
                if expert_tasks is not None and new_task_fric[-1][-1] in expert_tasks_clean:
                    continue
            ...
            if 'gravity' in self.rand_params:
                ...
                new_task_gravity = tuple(np.round(new_params['gravity'], 8))
                if expert_tasks is not None and new_task_gravity in expert_tasks_list:
                    continue
            ...
            if 'wind' in self.rand_params:
                ...
                new_task_wind = tuple(np.round(new_params['wind'], 8))
                if expert_tasks is not None and new_task_wind in expert_tasks_list:
                    continue
...
class Env(NonstationaryEnv):
    def __init__(self, env, rand_params=['gravity'], log_scale_limit=3, 
                 tasks_num=40, changing_interval=100, non_stationary=False):
        super().__init__(env, rand_params, log_scale_limit)
        
        self.non_stationary = non_stationary
        self.setted_env_changing_interval = changing_interval
        self.tasks = self.sample_tasks(tasks_num)
        
    def reset(self, **kwargs):
        self.cur_step_ind = 0
        self.task_ind = random.randint(0, len(self.tasks) - 1)
        self.set_task(self.tasks[self.task_ind])
        if self.non_stationary:
            env_param_list = [self.tasks[self.task_ind]] + [self.tasks[random.randint(0, len(self.tasks)-1)] for _ in range(15)]
            self.setted_env_params = env_param_list
        return super(NonstationaryEnv, self).reset(**kwargs)
    ...
    def step(self, action):
        ...
        if self.setted_env_params is not None and self.cur_step_ind % self.setted_env_changing_interval == 0:
            assert isinstance(self.setted_env_params, list)
            env_to_be = {}
            ind = self.cur_step_ind // self.setted_env_changing_interval
            if isinstance(self.setted_env_params[0], dict):
                for key in self.setted_env_params[0]:
                    env_to_be[key] = self.setted_env_params[ind][key]
            elif isinstance(self.setted_env_params[0], int):
                ...
                raise NotImplementedError(f'type of {type(self.setted_env_params[ind])} is not implemented.')
            self.set_task(env_to_be)
        res = super(NonstationaryEnv, self).step(self.denormalization(action))
        self._debug_state = res[0]
        return res


if __name__ == '__main__':
    from grid_world import RandomGridWorldPlat
